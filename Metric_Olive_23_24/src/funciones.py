...
def getRadiacionNeta(img_ee, roi, dem, lai_method, albedo_method, HR):
    ...
    # =========================
    # NUEVO: MÉTODO 5 (Q1-Ready)
    # MCARI2/OSAVI -> fPAR -> LAI
    # =========================
    if lai_method == 5:
        # Bandas Landsat 8/9 TOA
        NIR  = img_toa.select('B5')
        RED  = img_toa.select('B4')
        ...
        # MCARI2 (Haboudane 2004). Forma estándar:
        ...
        # Índice combinado recomendado en olivo
        idx = mcari2.divide(osavi.add(1e-6)).rename('MCARI2_OSAVI')

        # Paso 1: VI -> fPAR (lineal). Ajusta a y b con tus datos locales.
        # Coef. iniciales "conservadores" para olivo (ajústalos):
        a = 0.70   # pendiente (sube/baja el nivel)
        b = 0.05   # intercepto
        fpar = idx.multiply(a).add(b).clamp(0.01, 0.95).rename('fPAR')

        # Paso 2: fPAR -> LAI con Beer–Lambert: LAI = -ln(1 - fPAR)/k
        # k típico leñosos 0.3–0.6; arranca con 0.5 y calibra según tu seto/copa
        k = 0.5
        img_lai = fpar.expression(
            '-log(1 - f) / k', {'f': fpar.select('fPAR'), 'k': k}
        ).rename('LAI')

    # (opcional, al final, uniformiza rango físico)
